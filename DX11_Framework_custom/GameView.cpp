...
void GameView::UpdateView( const vector<Actor*>& actors, ILightSet * pLightSet ) const
{
	for each ( Actor* actor in actors )
	{
		drawModel( *actor, pLightSet );
	}
}
...
void GameView::drawModel( const Actor & actor, ILightSet * pLightSet ) const
{
	// UNTextured cube index is 0, so if > 0, use tex shader
	if( actor.GetModelType() > 0 )
	{
