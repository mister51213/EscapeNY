...
public:
	...
	// Game should be the class to initialize the list of actors, this class
	// should only initialize the graphics subsystem and accept a list of 
	// actor pointers to be rendered.
	...
	void DrawModel(
		ModelSpecs_W worldSpecs, 
		const std::shared_ptr<Model>& pMod )
	{
	...
	void UpdateView( 
		const vector<Actor *>& pActors,
		const vector<shared_ptr<Model>> &pModel )
	{
		for( int i = 0; i <= m_numModels; i++ )
		{
			DrawModel( pActors[ i ]->GetWorldSpecs(), pModel[i] );
		}
	}
